...
const createProductValidator = [
    ...
    body('priceConfiguration')
        .isObject()
        .withMessage('Product price configuration is required')
        .custom((priceConfiguration: { [key: string]: ProductConfig }) => {
            const keys = Object.keys(priceConfiguration);
            if (!keys.length) {
                throw new Error(
                    'price configuration must have alteast one key example:"Size, Crust',
                );
            }
            keys.forEach((key) => {
                const config = priceConfiguration[key];
                ...
                Object.entries(config.avialableOptions).forEach(([optionsKey, optionValue]) => {
                    ...
                    if (typeof optionValue !== 'number') {
                        throw new Error(
                            `${optionValue as number}.avilableOptions key must number `,
                        );
                    }
                });
            });
    ...
    body('attributes')
        ...
        .custom((attributes: ProductAttribute[]) => {
